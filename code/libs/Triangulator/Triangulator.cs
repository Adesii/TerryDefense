	...
	public static class Triangulator {
		...
		public static Vector2[] CutHoleInShape(Vector2[] shapeVerts, Vector2[] holeVerts) {
			...
			foreach(LineSegment segment in segmentsToTest) {
				float? intersection = segment.IntersectsWithRay(rightMostHoleVertex.Position, Vector2.Up);
			...
			Vector2 I = rightMostHoleVertex.Position + Vector2.Up * closestPoint.Value;
			...
			//to our rightMostHoleVertex, forms the line closest to Vector2.Up
			if(interiorReflexVertices.Count > 0) {
				float closestDot = -1f;
				foreach(Vertex v in interiorReflexVertices) {
					//compute the dot product of the vector against the Up
					Vector2 d = (v.Position - rightMostHoleVertex.Position).Normal;
					float dot = (float)Vector2.GetDot(Vector2.Up, d);
		...
		private static bool IsConvex(Vertex c) {
			...
			Vector2 d1 = (c.Position - p.Position).Normal;
			Vector2 d2 = (n.Position - c.Position).Normal;
			Vector2 n2 = new Vector2(-d2.y, d2.x);

			return (Vector2.GetDot(d1, n2) <= 0f);
		}
